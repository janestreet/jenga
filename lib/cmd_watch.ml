...
let show_error_state n (xs:Reportable.t) =
  puts "----------------------------------------------------------------------";
...
let watch_error_changes ~root_dir () : [`retry|`giveup of int] Deferred.t =
  Jenga_client.with_menu_connection_with_detailed_error ~root_dir ~f:(fun cwm ->
    ...
    show_error_pipe_changes ~initial ~updates ()
  ) >>| function
  | Ok () ->
...
let watch_errors =
  Command.async_or_error ~summary:"connect to running jenga, and watch the error pipe"
